...
def run_agent(config: dict, dry_run: bool = False, quota_override: int = None):
    ...
    special_criteria = config.get("special_criteria", "")
        ...
        while session_leads < quota:
            ...
            for comp in companies_raw:
                ...
                if dry_run:
                    ...
                    email_formats = search.deduce_email_format(
                        domain=comp_domain,
                        company_name=comp_name,
                        model=model,
                        delay=delay,
                ...
                console.print(f"      Likely patterns: [magenta]{', '.join(email_formats)}[/magenta]")

                # Insert company (store primary format)
                primary_format = email_formats[0] if email_formats else "first.last"
                ...
                for ex in execs_raw:
                    ...
                    # Build and verify email (Try each format until one works)
                    valid_email = None
                    for fmt in email_formats:
                        email_attempt = build_email(first, last, fmt, comp_domain)
                        if not email_attempt:
                            continue
                        
                        # Dedup check: email
                        if db.email_exists(email_attempt):
                            console.print(f"      [dim]SKIP (email exists): {email_attempt}[/dim]")
                            valid_email = email_attempt # Mark as found but don't re-save
                            break

                        # Deliverability check
                        if dry_run or verify_email_deliverability(email_attempt):
                            valid_email = email_attempt
                            break
                        else:
                            console.print(f"      [dim]Format check failed: {email_attempt}[/dim]")

                    if not valid_email:
                        console.print(f"      [yellow]⚠ No valid email found for {first} {last}[/yellow]")
                        continue

                    # Final check: was it already found?
                    if db.email_exists(valid_email):
                        continue
