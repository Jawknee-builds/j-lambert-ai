...
def search_companies(
    ...
    criteria_note = ""
    if special_criteria:
        criteria_note = f"\n\nADDITIONAL CRITERIA: {special_criteria}"

    user_prompt = f"""Find exactly 10 real companies in the {industry} industry located in or near {location} 
with approximately {size_min} to {size_max} employees.{criteria_note}
...
{exclusion_note}"""
    ...
    system_prompt = (
        ...
        "companies matching the user's specific targets and criteria. Always return valid JSON arrays. "
        "Never invent fake companies. Return ONLY the JSON array, no explanations."
    )
...
def deduce_email_format(
    ...
    """
    Ask Groq to figure out the most likely corporate email formats for a domain.
    Returns a list of patterns (e.g. ["first.last", "firstlast"]).
    """
    client = get_client()

    user_prompt = f"""What are the most likely corporate email formats used by {company_name} ({domain})?
...
Return ONLY a JSON object with a key "formats" containing a list of the top 2-3 most likely patterns in order of probability. Example:
{{"formats": ["first.last", "firstlast"]}}

If you cannot determine the format, return: {{"formats": ["first.last", "firstlast"]}}"""

    system_prompt = (
        "You are an email intelligence assistant. Determine the most likely corporate "
        "email formats for a given company domain. Return ONLY valid JSON, no explanations."
    )
    ...
    try:
        content = _call_llm(client, model, system_prompt, user_prompt)
        data = _extract_json(content)
        if isinstance(data, dict) and "formats" in data and isinstance(data["formats"], list):
            return data["formats"]
